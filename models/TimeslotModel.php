...
class TimeslotModel extends ActiveRecord
{
    ...
    /**
     * Deletes the Timeslots generated by this TimeslotModel between the given dates. Timeslots already booked aren't
     * removed, unless explicitly asked.
     * @param DateTime $from starting date (included) of the range (can't be in the past)
     * @param DateTime $to ending date date (included) of the range (can't be in the past)
     * @param bool $bruteDelete (optional, default false) ignore the booking status of the Timeslots being deleted
     * @return bool if the operation was successful
     */
    public function deleteTimeslots(DateTime $from, DateTime $to, $bruteDelete = false)
    {
        // Make sure that each day is fully included
        $from->modify('00:00:00');
        $to->modify('23:59:59');
        ...
        foreach ($timeslots as $timeslot) {
            // Booked Timeslots have to be explicitly removed
            if ( !$timeslot->isBooked() || $bruteDelete ) {
